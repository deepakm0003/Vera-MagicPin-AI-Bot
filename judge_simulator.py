...
# Choose your LLM provider: "nvidia", "openai", "anthropic", "gemini", "deepseek", "ollama", "openrouter"
LLM_PROVIDER = "nvidia"
...
class NVIDIAProvider(LLMProvider):
    def __init__(self, api_key: str, model: str = ""):
        self.api_key = api_key
        self.model = model or "google/gemma-4-31b-it"
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
    ...
    def _parse_response(self, response: str, action: Dict) -> ScoreResult:
        ...
        try:
            ...
            # Remove literal newlines inside strings that break JSON
            clean_json = re.sub(r'":\s*"(.*?)"', lambda m: '": "' + m.group(1).replace('\n', ' ') + '"', clean_json)
